...
final class ClauseConjunction
{
    ...
    /**
     * Negation of the current clause.
     */
    private ?self $negation = null;
     ...
     */
    public function __construct(array $clauses, bool $simplified = false)
    {
    ...
    public function andSimplified(self $other): self
    {
        ...
        return new self(self::simplifyCNF([
            ...$this->clauses,
            ...$other->clauses,
        ]), true);
    }
    /**
     * Merges the OR components of two CNF clause conjunctions.
     */
    public function combineOrredClauses(
        self $other,
        int $conditional_object_id
    ): self {
        $left_clauses = $this->clauses;
        $right_clauses = $other->clauses;
        ...
        if ($all_wedges) {
            return new self([new Clause([], $conditional_object_id, $conditional_object_id, true)], true);
        }
     ...
     * @psalm-pure
     */
    private static function simplifyCNF(array $clauses): array
    {
    ...
    public function getNegation(): ClauseConjunction
    {
        if ($this->negation !== null) {
            return $this->negation;
        }
        $this->negation = new self($this->makeNegation(), true);
        $this->negation->negation = $this;
        return $this->negation;
    }

    /** @return non-empty-list<Clause> */
    private function makeNegation(): array
    {
    ...
    /**
     * @param non-empty-array<Clause>  $clauses
     * @return list<Clause>
     * @psalm-pure
     */
    private static function groupImpossibilities(array $clauses): array
    {
        $complexity = 1;
        ...
        if (!$clause->wedge) {
            foreach ($clause->getNegation() as $var => $impossible_types) {
     ...
     */
    public function getTruthsFromFormula(
